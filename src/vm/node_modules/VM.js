    ...
    async.series([
        function (cb) {
            ...
            if (volume.hasOwnProperty('image_uuid')) {
                ...
                // 
                // Idea:
                // Clone from an existing snapshot in the dataset to provide 
                // moving of thinly provisioned zones from one server to another.
                // @final seems to exist in the smartos* datasets
    ...
    if (payload.hasOwnProperty('add_nics')) {
        for (n in payload.add_nics) {
            if (payload.add_nics.hasOwnProperty(n)) {
                ...
                if (nic.hasOwnProperty('primary')
                    && nic.primary === 1 || nic.primary === '1') {

                    VM.log('WARN', 'DEPRECATED: payload uses 1 instead of '
                        + 'true for nic.primary, use "true" instead.');
                    nic.primary = true;
                } else if (nic.hasOwnProperty('primary') && nic.primary === false) {
                    delete nic.primary;
                }
            }
        }
...
function normalizePayload(payload, vmobj, callback)
{
    ...
    if (payload.hasOwnProperty('disks')) {
        if (payload.hasOwnProperty('add_disks')) {
            callback(new Error('Cannot specify both "disks" and "add_disks"'));
            return;
        }
    ...
    if (payload.hasOwnProperty('nics')) {
        if (payload.hasOwnProperty('add_nics')) {
            callback(new Error('Cannot specify both "nics" and "add_nics"'));
            return;
        }
    ...
    if (vmobj) {
        ...
        /* create (also calls fixPayloadMemory() */
...
exports.update = function (uuid, payload, callback)
{
    ...
    async.series([
        function (cb) {
            ...
            p = {'disks': disks};
            p.uuid = uuid;
            if (vmobj.hasOwnProperty('zfs_storage_pool_name')) {
                p.zfs_storage_pool_name = vmobj.zfs_storage_pool_name;
            }
...
exports.info = function (uuid, types, callback)
{
    ...
    VM.load(uuid, function (err, obj) {
        ...
        if (obj.brand !== 'kvm') {
            callback(new Error('the info command is only supported for KVM VMs'));
            return;
        }
